<div class="max-w-4xl mx-auto p-6">
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg p-6">
        <div class="mb-6">
            <h2 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Processing Batches</h2>
        </div>
            ...
            <div class="space-y-4">
                @foreach($jobs as $job)
                    <div class="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-700 rounded-lg p-6">
                        ...
                        {{-- Download Button --}}
                        @if(($job['status'] === 'completed' || $job['status'] === 'completed_with_errors') && $job['download_available'])
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    Livewire.on('batchCreated', (batchId) => {
        if (typeof window.Echo !== 'undefined') {
            window.Echo.channel('batch.' + batchId)
                .listen('.job.progress.updated', (e) => {
                    console.log('Real-time update received for new batch:', e);
                    @this.call('updateJobProgress', batchId, e);
                });
        }
    });
